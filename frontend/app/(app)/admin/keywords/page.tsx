  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">Quản lý từ khóa tìm kiếm</h1>
          <p className="text-sm text-slate-500 mt-0.5">Từ khóa dùng để lọc video liên quan đến sản phẩm công nghệ</p>
        </div>
        <button
          onClick={openAdd}
      ...
      {clusters.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {clusters.map((c) => (
            <span key={c} className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-violet-100 text-violet-700">
              {c}
            </span>
          ))}
        </div>
            ...
            {loading ? (
              ...
              keywords.map((kw) => (
                ...
                >
                  ...
                  <td className="px-4 py-3.5"><StatusBadge active={kw.is_active} /></td>
