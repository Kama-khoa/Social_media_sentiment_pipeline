...
export default function ProductCandidatesPage() {
  ...
  const load = useCallback(async (nextStatus = status) => {
    setLoading(true);
    try {
      const [nextProducts, current, pending, approved, rejected] = await Promise.all([
        api.admin.products.list(),
        api.admin.products.candidates(nextStatus),
        api.admin.products.candidates("pending"),
        api.admin.products.candidates("approved"),
        api.admin.products.candidates("rejected"),
      ]);
