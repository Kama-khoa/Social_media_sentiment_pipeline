...
import { Donut, KpiCard, ScoreRing } from "@/components/shared/MockVisuals";
  ...
  return (
    <main className="fade-in mx-auto max-w-[1120px] px-6 py-7 pb-20">
      ...
      <section className="card mb-[18px] p-[26px]">
        <div className="flex flex-wrap justify-between gap-6"><div className="min-w-0 flex-[1_1_320px]"><div className="mb-2.5 flex flex-wrap gap-2"><span className="chip brand">{data.category}</span><span className="chip">{data.brand}</span><span className={`chip ${data.controversy_label === "high" ? "neg" : data.controversy_label === "medium" ? "neu" : "pos"}`}>{data.controversy_label === "high" ? "Nhiều tranh cãi" : data.controversy_label === "medium" ? "Tranh cãi vừa" : "Ít tranh cãi"}</span></div><h1 className="m-0 text-[32px] font-extrabold tracking-[-.02em]">{data.product_name}</h1><p className="muted mt-2 max-w-xl text-[14.5px] leading-relaxed">{data.details?.description ?? "Phân tích cảm xúc cộng đồng dựa trên bình luận YouTube Việt Nam."}</p><div className="mt-4 flex flex-wrap gap-2.5"><Button variant="secondary"><Icon name="heart" size={16} />Lưu sản phẩm</Button>{data.details?.official_url && <a href={data.details.official_url} target="_blank" rel="noreferrer"><Button variant="outline"><Icon name="external" size={16} />Trang chính thức</Button></a>}</div></div><div className="flex min-w-48 flex-col items-center gap-3"><ScoreRing score={data.bayesian_score} size={104} /><div className="text-center"><div className="num text-[15px] font-bold">{data.total_mentions.toLocaleString("vi-VN")}</div><div className="faint text-xs">lượt đề cập đã phân tích</div></div></div></div>
