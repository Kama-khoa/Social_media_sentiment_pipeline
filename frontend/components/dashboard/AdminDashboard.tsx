...
export function AdminDashboard() {
  ...
  return <div className="flex flex-col gap-5">
    ...
    <div className="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-3.5"><KpiCard icon="play" label="Video thu thập hôm nay" value={quick_stats.videos_today.toLocaleString("vi-VN")} /><KpiCard icon="bell" label="Bình luận thu thập hôm nay" value={quick_stats.comments_today.toLocaleString("vi-VN")} spark={mentionSeries.length ? mentionSeries : undefined} accent="var(--v-400)" /><KpiCard icon="box" label="Sản phẩm theo dõi" value={quick_stats.products_tracked.toLocaleString("vi-VN")} sub={`${user.category_stats.length} danh mục từ BigQuery`} accent="var(--pos)" /><KpiCard icon="broadcast" label="Kênh đang hoạt động" value={quick_stats.active_channels.toLocaleString("vi-VN")} sub={`${quick_stats.active_keywords.toLocaleString("vi-VN")} từ khóa`} accent="var(--neu)" /></div>
    {(admin.attention_items.length > 0 || admin.recent_dag_runs.length > 0) && <div className="grid gap-[18px] lg:grid-cols-[1fr_1fr]">
      <section className="card p-[22px]"><h3 className="mb-3.5 text-base font-bold">Tình trạng cần chú ý</h3>{admin.attention_items.length ? <div className="flex flex-col gap-2.5">{admin.attention_items.map((item, index) => <div key={`${item.message}-${index}`} className={`rounded-lg border px-3.5 py-2.5 text-sm ${item.level === "warning" ? "border-[var(--neg)] bg-rose-50 text-[var(--neg)]" : "border-[var(--border)] bg-[var(--surface-2)] text-[var(--text-2)]"}`}>{item.message}</div>)}</div> : <div className="faint text-sm">Chưa có cảnh báo vận hành.</div>}</section>
      <section className="card p-[22px]"><h3 className="mb-3.5 text-base font-bold">DAG runs gần đây</h3>{admin.recent_dag_runs.length ? <div className="flex flex-col gap-2.5">{admin.recent_dag_runs.map((run) => <div key={`${run.dag_id}-${run.run_id}`} className="flex items-center justify-between gap-3 rounded-lg bg-[var(--surface-2)] px-3.5 py-2.5 text-sm"><span className="min-w-0 truncate font-mono text-xs">{run.dag_id}</span><span className={`chip ${run.state === "success" ? "pos" : run.state === "failed" ? "neg" : "brand"}`}>{run.state}</span></div>)}</div> : <div className="faint text-sm">Chưa có DAG run gần đây hoặc Airflow chưa sẵn sàng.</div>}</section>
    </div>}
    ...
    <section className="card overflow-hidden p-[22px]"><h3 className="mb-3.5 text-base font-bold">Top sản phẩm theo điểm Bayes</h3><div className="overflow-x-auto"><table className="tbl"><thead><tr><th>#</th><th>Sản phẩm</th><th>Điểm</th><th className="min-w-32">Cảm xúc</th><th>Đề cập</th><th>Tranh cãi</th></tr></thead><tbody>{user.top_products.slice(0, 5).map((item) => <tr key={item.product_id}><td className="num font-bold text-[var(--primary)]">{item.rank}</td><td className="font-bold">{item.product_name}</td><td className="num font-bold">{hasEnoughBayesData(item.statement_count) ? bayesScore100(item.bayesian_score).toFixed(1) : "Chưa đủ dữ liệu"}</td><td><SentimentBar positivePct={item.positive_pct} negativePct={item.negative_pct} height={8} /></td><td className="num muted">{item.total_mentions.toLocaleString("vi-VN")}</td><td><ControversyBadge label={item.controversy_label} /></td></tr>)}</tbody></table></div></section>
  </div>;
}
